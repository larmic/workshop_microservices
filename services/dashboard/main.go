...
func main() {
	...
	bookingRefStory8URL := getEnv("BOOKING_REF_STORY8_URL", "http://booking-ref-story8:8080")
	...
	bookingURLs := map[string]string{
		...
		"booking-ref-story8": bookingRefStory8URL,
	}
	if bookingCustomURL != "" {
	...
	mux.HandleFunc("POST /api/booking-ref-story3/bookings", handler.ProxyHandler(bookingRefStory3URL, http.MethodPost, "/booking/bookings"))
	mux.HandleFunc("GET /api/booking-ref-story4/offers", handler.ProxyHandler(bookingRefStory4URL, http.MethodGet, "/booking/offers"))
	mux.HandleFunc("GET /api/circuit-state", handler.CircuitStateHandler(bookingRefStory4URL))
	mux.HandleFunc("GET /api/bulkhead-state", handler.BulkheadStateHandler(bookingRefStory5URL))
	mux.HandleFunc("POST /api/bulkhead-reset", handler.BulkheadResetHandler(bookingRefStory5URL))
	mux.HandleFunc("GET /api/booking-ref-story5/offers", handler.BookingOffersProxyHandler(bookingRefStory5URL))
	mux.HandleFunc("POST /api/booking-ref-story5/burst", handler.BurstHandler(bookingRefStory5URL, 20))
