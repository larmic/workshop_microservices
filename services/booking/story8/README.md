# Booking Service — Story 8: Distributed Tracing

## Was diese Story zeigt

Story 8 baut auf der Choreography-Saga aus Story 7 auf und ergänzt sie um
...
## Was technisch neu ist gegenüber Story 7

| Aspekt | Story 7 | Story 8 |
|---|---|---|
...
```bash
curl -X POST http://localhost/api/booking-ref-story8/booking/bookings \
...
```bash
docker compose logs booking-ref-story8 flight hotel car | jq -c 'select(.trace_id=="<trace-id>")'
```
...
Bei der Compensation aus Story 7 schickt Booking ein `CompensationRequested`-
