...
Systemen die schwierigen sind — und die Brücke zu Story 7
...
Story 6. Pragmatisch ist die Entscheidung trotzdem vertretbar — *wenn*
...
über sync HTTP** (Story 6) zu **Event-getriebener Saga** (Story 7).
...
verschiebt die Komplexität, sie eliminiert sie nicht. Story 6 zeigt
die Saga **isoliert** (sync, ein Konzept). Story 7 schaltet das
...
Infrastruktur aus Story 7 *nutzen*, muss aber nicht — denn CQRS
...
Bei **Choreography-Saga** (Story 7) wandert ein Stück Saga-Wissen in die
...
DB-Schicht** — auch Story 1–5 nicht. Wenn wir Persistenz workshop-
konform nachziehen wollten, wären die kleinsten Schritte:

1. **SQLite-Datei im Container** — eine Datei, kein neues Infra-Stück.
2. **Consul KV nutzen** — Consul ist seit Story 3 im Stack. Saga-State
...
- Brücke zu Story 7: Welcher Teil der heutigen Saga-Implementierung
